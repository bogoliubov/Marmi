...
Se $x\in\omega(x)$ (risp. $x\in\alpha(x)$) si dice positivamente (risp. negativamente) ricorrente, e nel caso accadano entrambe le cose si dice che $x$ è ricorrente, 
...
\end{lemma}
\begin{proof}
 Se $\{g_0^kg_1\}_k$ è densa, poichè $g_0^kg=(g_0^kg_1)(g_1^{-1}g)$ si ha che l'orbita di $g$ si ottiene con una traslazione, che per ipotesi è un omeomorfismo, di un'orbita densa. Per il secondo punto, abbiamo che il sottogruppo ciclico $G'=\{g_0^kg_1\}_k$ è denso, quindi poichè lo spazio è metrico si usa il criterio di continuità per successioni: se $g_n\rightarrow g$ e $h_n\rightarrow h$ con $g_n, h_n \in G'$, si ha $[g,h]\leftarrow [g_n,h_n]=e$, e quindi per continuità $[g,h]=e$.
\end{proof}
...
\[ \xymatrix{ \{0,1\}^\N \ar[r]^\sigma \ar[d]_\pi & \{0,1\}^\N \ar[d]^\sigma \\ [0,1] \ar[r]_S & [0,1] } \]
 dove $S(x)=2x(\mbox{mod } 1)$ è il \emph{raddoppiamento}, $\sigma(a_1,a_2,\dots)=(a_2,a_3,\dots)$ è lo \emph{shift} e 
...
\begin{esempio}[Tenda e Logistic Map] Un altro esempio di dinamiche coniugate: su $[0,1]$ sono coniugate tramite $h(x)=\sin^2\left(\frac{2\pi}{2}\right)$ la mappa \emph{a tenda} $f(x)=\begin{cases}2x \quad x\leq 1/2 \\ 2-2x \quad x\geq 1/2\end{cases}$ e la \emph{mappa logistica} $Q_4(x)=4x(1-x)$, $Q_4\circ h=h\circ f$.
\end{esempio}
...
\begin{enumerate}
	...
	\item per ogni aperto $U$ $f$-invariante (cioè con $U=f(U)=f^{-1}(U)$) è $U=\emptyset$ oppure $\obar{U}=X$
	\item per ogni $U,V$ aperti non vuoti esiste $N\in\Z$ tale che $f^N(U)\cap V\neq\emptyset$
...
\begin{oss}
\begin{itemize}
 \item Se $f:X\to X$ è solo continua (cioè è un endomorfismo), è facile dimostrare che le condizioni $(1)$ e $(3)$ sono
  ancora equivalenti se assumiamo che $X$ non abbia punti isolati.
 \item Nel caso $f\in\homeo (X)$ la terza condizione del teorema è equivalente alla \emph{decomponibilità}: 
...
funzioni $F:X\to\R$ continue tali che $F\circ f=F$) sono le costanti.
\end{prop}
...
\begin{esempio}Il viceversa in generale è falso! Sia $T:=\R^2/\Z^2$ il $2$-toro e $A\in SL(2,\Z)$ una trasformazione ergodica
su $T$ (ad esempio il \emph{gatto di Arnold},
cioè $A=\begin{pmatrix} 2 & 1 \\ 1 & 1 \end{pmatrix}$). Costruiamo $X$ partendo da due copie del toro, $T$ e $T'$,
e identificando i loro elementi neutri. La mappa su $T\sqcup T'$ che si ottiene accostando $A:T\to T$
e $A:T'\to T'$ induce una mappa $f:X\to X$. \\
...
come pure $T'$).
Un integrale primo $F:X\to \R$ però è costante: infatti $\restr{f}{T}:T\to T$ è topologicamente transitivo (v. ??),
perciò $F$ è costante su $T$ e lo stesso vale per $T'$. Siccome (in $X$) $T$ e $T'$ si intersecano, deduciamo che $F$ è costante su tutto $X$.
\end{esempio}

\begin{esempio}La tenda $T:[0,1]\to [0,1]$, $T(x):=\begin{cases}2x &\text{se }0\le x\le\mz \\ 2-2x &\text{se }\mz\le x\le 1\end{cases}$
è transitiva. Infatti per gli intervalli della forma $\bra{\frac{i}{2^k},\frac{i+1}{2^k}}$ l'immagine
tramite $T^n$ è definitivamente tutto $[0,1]$ (si dice che $T$ è \emph{espansiva}).
\end{esempio}

\begin{oss}Se $g$ è un fattore di $f$ e $f$ è topologicamente transitiva, allora lo è anche $g$.
\[ \xymatrix{ X \ar[r]^f \ar[d]_h & X \ar[d]^h \\ Y \ar[r]_g & Y } \]
Infatti per qualche $h:X\to Y$ surgettiva è $gh=hf$ (omettiamo il $\circ$), quindi $g^k h=h f^k$, perciò
se $x\in X$ ha orbita densa abbiamo $O_g(h(x))=h(O_f(x))$, che è un insieme denso essendo $h$ surgettiva.
\end{oss}

\begin{esempio}Siano $X:=[0,1]$ e
$S(x):=\begin{cases}2x &\text{se }0\le x<\mz \\ 2x-1 &\text{se }\mz\le x\le 1\end{cases}$
il ``dente di sega''\footnote{Questo esempio è un po' improprio perché $S$ è discontinua}.
Il seguente diagramma commuta:
\[ \xymatrix{X \ar[r]^S \ar[d]_T & X \ar[d]^T \\ X \ar[r]_T & X} \]
e questo mostra che $T$ è un fattore di $S$.
\end{esempio}

?? perché questo esempio?

\begin{defi}Siano $X$ spazio metrico compatto e $f:X\to X$ continua. $f$ è \emph{Devaney-caotica}
se è topologicamente transitiva e $\obar{\per(f)}=X$.
\end{defi}

In questa definizione $\per(f)$ è l'insieme dei punti periodici.
...
Prima della dimostrazione vediamo un lemma molto semplice.
...
\begin{proof}[Dimostrazione del teorema]Se ci fosse una sola orbita periodica,
da $\obar{\per(f)}=X$ seguirebbe che $X$ è finito e quindi tutti i punti sarebbero isolati, assurdo.
Perciò siamo nelle ipotesi del lemma. Poniamo $\delta:=\frac{c}{4}$ ($c$ è la costante che ci fornisce il lemma). \\
Sia $x\in X$ e $U'$ un suo intorno; definiamo $U:=U'\cap B(x,\delta)$. Per la densità di $\per(f)$ esiste $q\in U$ periodico,
diciamo di periodo $n$. \\
Sappiamo (lemma) che c'è un punto $p$ periodico con $d\pa{x,O_f(p)}>c=4\delta$. Inoltre per transitività
