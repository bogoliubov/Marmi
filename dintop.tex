...
\begin{defi} $x\in X$ è un \emph{punto errante} se esiste un suo intorno $U$ tale che 
\[\bigcup_{n\neq 0} f^n(U)\cap U=\varnothing.\]
L'insieme di tali punti è detto l'\emph{insieme errante} del sistema. Un insieme $A$ è \emph{invariante} se $f(A)\subseteq A$;
per omeomorfismi si dice \emph{totalmente invariante} se lo è anche negativamente, cioè per $f^{-1}$, ovvero si ha $f^{-1}(A)=A=f(A)$. 
Gli insiemi di $\alpha$ e $\omega$-limite ($\alpha$ solo per omeomorfismi) sono
...
Se $x\in\omega(x)$ (risp. $x\in\alpha(x)$), $x$ si dice positivamente (risp. negativamente) ricorrente. Nel caso degli endomorfismi, si dice semplicemente che $x$ è \emph{ricorrente}, 
il che equivale a $x$ non errante. I punti ricorrenti sono quindi l'insieme non errante.
\end{defi}
...
\begin{oss}\label{osstransitivita} Se $f:X\to X$ è solo continua (cioè è un endomorfismo), è facile dimostrare che le condizioni $(1)$ e $(3)$ sono
  ...
  $f$ non è transitiva se e solo se esistono due aperti disgiunti (totalmente) invarianti.
\end{oss}
...
\begin{proof}
 ...
 Per il secondo punto, abbiamo che il sottogruppo ciclico (e quindi abeliano) $G'=\{g_0^k\mid k\in\Z\}$ è denso, ma poichè lo spazio è metrico si può usare il criterio di continuità per successioni: 
...
\[ \xymatrix{ \{0,1\}^\N \ar[r]^\sigma \ar[d]_\pi & \{0,1\}^\N \ar[d]^\pi \\ [0,1] \ar[r]_S & [0,1] } \]
 ...
 La mappa $\pi$ è surgettiva e dunque $S$ risulta essere un fattore di $\sigma$. In questo modo si vede subito che il raddoppiamento è transitivo non minimale: 
...
su $[0,1]$ la mappa \emph{tenda} 
$f(x)=\begin{cases}2x & x\leq 1/2 \\ 2-2x & x\geq 1/2\end{cases}$ e la \emph{mappa logistica} $Q_4(x)=4x(1-x)$
sono coniugate tramite $h(x)=\sin^2\left(\frac{\pi x}{2}\right)$, cioè $Q_4\circ h=h\circ f$.
...
è il dente di sega, allora commuta il diagramma
...
\begin{esempio}[facoltativo] Avere solo integrali primi costanti non implica la transitività. Sia $T:=\R^2/\Z^2$ il $2$-toro e $A\in SL(2,\Z)$ una trasformazione ergodica
...
come pure $T'$).
Un integrale primo $F:X\to \R$ però è costante: infatti $\restr{f}{T}:T\to T$ è topologicamente transitivo 
(essendo ergodico, vedi l'Esempio \ref{gattoerg} e l'Osservazione \ref{ergtrans}),
...
\begin{defi}$(X,d,f)$ ha \emph{dipendenza sensibile dai dati in} $x_0\in X$ se 
 ...
 (contronominale della precedente) ovvero se le iterate $\{f^n\}$ sono equicontinue in $x_0$.
\end{defi}
