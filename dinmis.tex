...
cioè tale che $f^{-1}\A\subseteq \A$ (ossia $f$ misurabile) e per ogni $A\in\A$ valga $\mu(f^{-1}A)=\mu(A)$. 
(Scegliamo questa definizione con $f^{-1}$ coerentemente con la nozione di misura immagine.)
...
Dato $A\in\A$, coerentemente con la nozione di punto ricorrente in dinamica topologica, definiamo l'insieme dei punti ricorrenti di $A$ come
\[A_r=\{x\in A: f^n(x)\in A \mbox{ frequentemente}\}.\]
\begin{teo}[ricorrenza di Poincarè] Per ogni $a\in\A$ vale $\mu(A\smallsetminus A_r)$.\end{teo}
\begin{proof} I punti non ricorrenti sono quelli che definitivamente non tornano in $A$, per cui abbiamo
\[A_r=A\smallsetminus \bigcup^\infty B_n \quad \mbox{con} \quad B_n=A\smallsetminus \bigcup_{j\geq n}f^{-j}A\]
da cui intanto abbiamo $A_r$ misurabile, inoltre i punti di $B_n$ sono punti che tornano in $A$ ma non dopo $n$, in simboli
\[B_n\subseteq \bigcup_{j\geq 0} f^{-j}(A)\smallsetminus \bigcup_{j\geq n} f^{-j}(A)\]
(nella prima unione è contenuto anche $A$) da cui 
\[\begin{aligned}\mu B_n &\leq \mu\left(\bigcup_{j\geq 0} f^{-j}(A)\right)-\mu\left(\bigcup_{j\geq n} f^{-j}(A)\right)\\
			 &=\mu\left(\bigcup_{j\geq 0} f^{-j}(A)\right)-\mu\left(f^n \bigcup_{j\geq 0} f^{-j}(A)\right)=0
  \end{aligned}\]
in cui l'ultimo passaggio si ha perchè $f$ preserva la misura.
\end{proof}

Una bigezione che coniuga un sistema misurabile $(X,\A,\mu,f)$ con uno qualsiasi $Y$ induce su $Y$ una misura invariante:
basta fare il pushforward di $\mu$ tramite il coniugio. \Eacc questo il caso dei seguenti esempi.
...
nella circonferenza unitaria. Restringendo il coniugio al bordo di questi due domini, stiamo coniugando la nostra $f$ originale con il raddoppiamento su $S^1$, 
e con un semplice calcolo si ricava che il coniugio è $S^1\ni\theta\mapsto -\cot \frac{\theta}{2}\in\R$; trasportando la misura di Lebesgue da $S^1$
a $\R$ si ha quanto detto. 
\end{esempio}
...
Dimostreremo più avanti il seguente risultato (ci restringeremo alla prima parte):
...
cioè \emph{la statistica delle orbite coincide con la probabilità a priori}.
\end{defi}

\begin{teo}Sono equivalenti:
\begin{lista}
	\item $X$ ergodico 
	\item per ogni $A\in\mathcal{A}$ invariante, cioè con $f(A)\subseteq A$, uno tra $A$ e $A^c$ è trascurabile 
...
\begin{oss}\label{ossergodicita}Notiamo che \begin{itemize}
    \item Se un insieme $A\in\mathcal{A}$ è ``quasi invariante'', nel senso che $f(A)\subseteq A\cup N$ per qualche
      ...
      basta porre $N':=\bigcup_{j=0}^\infty f^{-j}(N)$ e $A':=A\setminus N'$. Ora se $x\in A'$ abbiamo
...
\begin{oss}[facoltativa]\label{ergtrans} Sia $(X,d,f)$ uno spazio metrico compatto con una trasformazione $f:X\to X$ continua che preserva
...
\begin{teo}[ergodico di von Neumann] Sia $U$ un'isometria di $H$ spazio di Hilbert complesso; allora
 \[\forall\phi\in H \quad \exists \lim_{n\rightarrow\infty}\frac{1}{n}\sum_{j=0}^{n-1}U^j\phi=:\hat\phi\]
 e inoltre $U\hat\phi=\hat\phi$. Per cui in una dinamica misurabile $(X,\A,\mu,f)$, considerando nello spazio di Hilbert $L^2_\C(X,\A,\mu)$ l'\emph{operatore di Koopman} $U_f$ dato da $U_f \phi=\phi\circ f$ (che è un'isometria essendo $\mu$ $f$-invariante), si ha che le somme di Birkhoff convergono in $L^2$. 
 \end{teo}
...
\begin{oss}
 ...
 Per applicare questo ragionamento in dimensione infinita servirebbe il teorema spettrale relativo (di Stone). La dimostrazione che daremo è elementare e si deve a Riesz.
\end{oss}

\begin{proof}
 Sia $D=\{\phi\in H|\exists\psi\in H : \phi=U\psi-\psi\}$ (sottospazio dei \emph{cobordi}); anzitutto mostriamo che $\forall \theta \in D^\bot \quad U\theta=\theta$. 
 ...
 Sia ora $I=\{\phi\in H | U\phi=\phi\}$; abbiamo $(D\cup I)^\bot=\{0\}$, quindi $D\cup I$ è denso in $H$. 
 ...
 Sia $\phi_k\rightarrow\phi$ di Cauchy in $D\cup I$, ovvero $h,k$-definitivamente $\|\phi_h-\phi_k\|\le\epsilon$; allora anche $\|\hat\phi_h-\hat\phi_k\|\le\epsilon$ (perché le somme di Birkhoff sono $1$-Lipschitz), 
...
inoltre saranno $\nu(x,A,n):=\frac{1}{n}T(x,A,n)$, $\bar\nu(x,A):=\limsup_{n\rightarrow\infty}\frac{1}{n}T(x,A,n)$ e analogamente $\underline\nu(x,A):=\liminf_{n\rightarrow\infty}\frac{1}{n}T(x,A,n)$ e infine, se esiste, $\nu(x,A):=\lim_{n\rightarrow\infty}\frac{1}{n}T(x,A,n)$.
\end{defi}
...
\begin{proof}
 ...
 e analogamente $\underline\tau$ per $\underline\nu$. Supponiamo dapprima che $\forall x\in X \quad \bar\tau(x,A,\epsilon)\leq M$ (con $M$ che dipende da $\epsilon$). Fissiamo $n$ più grande di $M$ e definiamo ricorsivamente una sottosuccessione di $\{S^jx\}_0^{n-1}$
 \[\begin{cases}
 x_0=x & \tau_0=\bar\tau(x_0,A,\epsilon)\\
 x_{k+1}=S^{\bar\tau(x_k,A,\epsilon)}x_k=S^{\tau_k}x_0 & \tau_k=\sum_{h=0}^{k-1}\bar\tau(x_h,A,\epsilon)
 \end{cases} \]
 che quindi deve concludersi con $x_{K}$ tale che $\tau_{K}<n$ e $\tau_{K+1}\geq n$. Osserviamo che, poichè $\nu(x,A,\bar\tau(x,A,\epsilon))\geq\bar\nu(x,A)-\epsilon$ e $\bar\nu(x,A)=\bar\nu(Sx,A)$, si hanno le seguenti disuguaglianze
 \[\begin{split}
 ...
 \[\begin{split}
  T(x,A,n)&=\sum_{k=0}^{K-1}T(x_k,A,\bar\tau(x_k,A,\epsilon))+\underbrace{T(x_k,A,n-K)}_{\geq 0}   \\
	  &\geq \underbrace{\sum_{k=0}^{K-1} \bar\tau(x_k,A,\epsilon)}_{\tau_K}(\bar\nu(x,A)-\epsilon)   \\
	  &\geq (n-M)(\bar\nu(x,A)-\epsilon)
 \end{split}\]
da cui, integrando, si ha
\[n \mu(A)=\int_X T(x,A,n) d\mu(x)\geq (n-M)\left(\int_X \bar\nu(x,A)d\mu(x)-\epsilon\right)\]
e dunque, mandando $n\to\infty$ e per l'arbitrarietà di $\epsilon$, abbiamo $\mu(A)\geq \int_X \bar\nu(x,A)d\mu(x)$. Analogamente, supponendo $\underline\tau$ limitato, si ottiene l'altra disuguaglianza:
...
\[\bar\tau'(x,A,\epsilon)=\begin{cases}\bar\tau(x,A,\epsilon) & x\notin A_C\\
					1 & x\in A_C
  \end{cases}\]
...
\[T(x_k',\bar A,\bar\tau'(x_k',A,\epsilon))
    \begin{cases} =T(x_k',\bar A,1)=1\geq \bar\nu(x_0,A)-\epsilon & x_k'\in A_C \\
		  \begin{aligned}\geq T(x_k',A,\bar\tau'(x_k',A,\epsilon))&=T(x_k',A,\bar\tau(x_k',A,\epsilon))\\
			&\geq \bar\tau(x_k',A,\epsilon)(\bar\nu(x_0,A)-\epsilon)\end{aligned} & x_k'\nin A_C 
    \end{cases}\]
\[\Rightarrow T(x_k',\bar A,\bar\tau'(x_k',A,\epsilon))\geq \bar\tau'(x_k',A,\epsilon)(\bar\nu(x_0,A)-\epsilon)\]
...
Se $(\mu_1-\mu)(A)=M$ allora $A$ coincide con $P$ a meno di insiemi $\abs{\mu_1-\mu}$-trascurabili e scriveremo $A\sim P$.
...
cascano frequentemente in $N$). $A$ è $S$-invariante e $\mu(A)=0$, $\mu_1(A)\neq 0$ grazie all'invarianza di $\mu$ e $\mu_1$.
\end{oss}

\begin{oss}Se $\mu_1\neq\mu$ è anch'essa ergodica, si ha $\mu_1(P')=1$, quindi $\mu\perp\mu_1$.
\end{oss}

\begin{defi}$(X,\mathcal{A},\mu,S)$ è \emph{unicamente ergodico} se $\mu$ è l'unica misura di probabilità $S$-invariante su $\A$.
\end{defi}
...
Inoltre, tornando a una vecchia osservazione sul decadimento delle correlazioni, il teorema afferma che è infinitesimo
...
\begin{esempio}\label{gattoerg} Gli automorfismi lineari \emph{iperbolici} di $\mathbb{T}^2=\R^2/\Z^2$, cioè
